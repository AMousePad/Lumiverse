...
describe('HomepageCharacterLibrary preview overflow contract', () => {
  ...
  test('keeps the entire preview image visible without a blurred duplicate', async () => {
    const css = await Bun.file(new URL('./HomepageCharacterLibrary.module.css', import.meta.url)).text()
    ...
    expect(previewFrame).toMatch(/height:\s*var\(--homepage-panel-image-height,\s*320px\)/)
    ...
    expect(previewImage).toMatch(/object-fit:\s*contain !important/)
    expect(previewImage).toMatch(/object-position:\s*center !important/)
  })
  ...
  test('fits each image from intrinsic dimensions and the remaining metadata space', async () => {
    ...
    expect(tsx).toMatch(/getHomepagePreviewAvailableImageHeight\(body\.clientHeight, metadata\.offsetHeight, rowGap\)/)
    ...
    expect(tsx).toMatch(/max=\{panelImageHeightMax\}/)
    expect(tsx).toMatch(/<span>\{panelImageHeight\}px<\/span>/)
