...
async function runConsolidationCheck(
  ...
  if (!(config.useSidecar && generateRawFn && sidecarConnectionId)) return false;
  ...
  let title: string | null;
  {
    ...
    let result: GeneratedSummary | null;
    try {
      ...
      if (err?.name === "AbortError" || ac?.signal.aborted) {
        console.warn(`[memory-cortex] Consolidation sidecar timed out after ${timeoutMs}ms; source chunks remain unconsolidated.`);
        result = null;
      } else {
        throw err;
      }
    ...
    // Do not replace source chunks with a transcript-like fallback.
    if (!result) {
      return false;
    }
...
async function maybeConsolidateArcs(
  ...
  if (!(config.useSidecar && generateRawFn && sidecarConnectionId)) return;
  ...
  let arcTitle: string | null;
  {
    ...
    try {
      ...
      if (err?.name === "AbortError" || ac?.signal.aborted) {
        console.warn(`[memory-cortex] Arc consolidation sidecar timed out after ${timeoutMs}ms; source scenes remain unconsolidated.`);
        result = null;
      } else {
        throw err;
      }
...
async function generateConsolidationSummary(
  ...
  const content = chunks
    ...
    .join("\n\n---\n\n");
  return generateSemanticSummary("scene", content, generateRawFn, connectionId, maxTokens, samplingParameters);
}
...
async function generateArcSummary(
  ...
  return generateSemanticSummary("arc", combinedSummaries, generateRawFn, connectionId, maxTokens, samplingParameters);
}
